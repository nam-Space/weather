...
function getweather(a, b) {
    fetch(
        ...
        .then(function show() {
            ...
            feelEl.innerHTML = `<i class="fa-solid fa-temperature-three-quarters"></i><span>Real Feel: </span><span>${weather.feel}&#176;C</span>`;
            windEl.innerHTML = `<i class="fa-solid fa-wind"></i><span>Wind: </span><span>${weather.wind}km/h</span>`;
            sunEl.innerHTML = `<i class="fa-solid fa-temperature-three-quarters"></i><span>Sunrise: </span><span>${weather.sun}</span>`;
            setEl.innerHTML = `<i class="fa-solid fa-wind"></i><span>Sunset: </span><span>${weather.set}</span>`;
    ...
    fetch(
        ...
        .then(function (data1) {
            ...
            for (var i = 0; i < 40; i++) {
                ...
                if (dmy === lists.gettom[i]) {
                    lists.temperature[k] = Math.floor(
                        data1.list[i].main.temp - kelvin
                    );
        ...
        .then(function show() {
            ...
            if (desc === "moderate rain")
                imagebg.src = "./asset/moderate rain.jpeg";
            ...
            for (let i = 0; i < 6; i++) {
                ...
                imgloop[
                    i
                ].innerHTML = `<img src="asset/${lists.img[u]}.png" alt="" style="width: 100%"/>`;
            ...
            for (let i = 0; i < 4; i++) {
                t2[
                    i
                ].innerHTML = `<img class="icon"src="asset/${lists.imageicon[t]}.png" alt="" style="width: 100%">  <p class="p">${lists.des5[t]}</p>`;
                t = t + 7;
            }
        });
}
...
function getreaschcity(a) {
    fetch(
        `https://api.openweathermap.org/data/2.5/forecast?q=${a}&appid=${key}`
    )
        .then(function (res) {
            let data1 = res.json();
            return data1;
        ...
        .then(function show() {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                imgloop[
                    i
                ].innerHTML = `<img src="asset/${lists.img[u]}.png" alt="" />`;
                // imgloop[i].innerHTML = `${lists.img[i]}.png`;
                u++;
            }
            for (let i = 0; i < 5; i++) {
...
const container = document.querySelector(".hole-container");
